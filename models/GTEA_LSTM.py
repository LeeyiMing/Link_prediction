...
class GTEALSTMLayer(nn.Module):
    def __init__(self, node_in_dim, node_hidden_dim, edge_in_dim, num_time_layers, bidirectional, drop_prob, device):
        super(GTEALSTMLayer, self).__init__()
        ...
        self.edge_layer = LSTM(edge_in_dim, node_hidden_dim, num_time_layers, device, bidirectional=bidirectional)
        self.edge_attn_layer = LSTM(edge_in_dim, node_hidden_dim, num_time_layers, device, bidirectional=bidirectional)
...
class GTEALSTM(nn.Module):
    def __init__(self, num_nodes, node_in_dim, node_hidden_dim, edge_in_dim, num_class, num_layers, num_time_layers, bidirectional, device, drop_prob=0):
        super(GTEALSTM, self).__init__()
        ...
        self.gcn_layers.append(GTEALSTMLayer(node_in_dim, node_hidden_dim, edge_in_dim, num_time_layers, bidirectional, drop_prob, device))

        for i in range(1, num_layers):
            self.gcn_layers.append(GTEALSTMLayer(node_hidden_dim, node_hidden_dim, edge_in_dim, num_time_layers, bidirectional, drop_prob, device))
